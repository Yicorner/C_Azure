...
void RendererGUI::run_only_save_image(std::string fn, std::string ext)
{
    volren.saveImage(fn, ext);
}
...
void RendererGUI::run()
{
    ...
    volren.framebuffer_size = glm::vec2(glfw_manager.framebuffer_width, glfw_manager.framebuffer_height);
    volren.setup();
    glfw_manager.setCameraUpdateCallback(std::bind(&decltype(volren.main_cam)::setOrientation, 
                                               &volren.main_cam,
                                               std::placeholders::_1,
                                               std::placeholders::_2,
                                               std::placeholders::_3));
    ...
    while(!glfwWindowShouldClose(glfw_manager.window))
    {
        ...
        // qbh获取鼠标位置
        glfwGetCursorPos(glfw_manager.window, &mouseX, &mouseY);
...
void RendererGUI::showProfiler()
{
    ...
    if (ImGui::Begin("Profiler##window", &profiler_shown, ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoFocusOnAppearing | ImGuiWindowFlags_NoNav))
    {
        ...
        //qbh
		float width = glfw_manager.window_width;
		float height = glfw_manager.window_height;
        ImGui::Text("origin X %.6f", mouseX);
        ImGui::Text("origin Y %.6f", mouseY);
        ImGui::Text("NDC X: %.6f", ((float)mouseX - width / 2) / (height / 2));
        ImGui::Text("NDC Y: %.6f", -((float)mouseY - height / 2) / (height / 2));
        ImGui::End();
    }
    ImGui::PopStyleColor();
}
...
void RendererGUI::showTools()
{
    ...
    if (ImGui::Begin("Tools##window", &tools_shown, ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoFocusOnAppearing | ImGuiWindowFlags_NoNav))
    {
        ...
        // qbh
        if (ImGui::Button("rotation horizon 90 degree"))
        {
            // 绕 Y 轴旋转 90 度
			float rot_speed = volren.main_cam.rotation_speed;
            volren.main_cam.setOrientation(0.0f, 0.0f, glm::half_pi<float>() / rot_speed);
        }
