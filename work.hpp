...
public:
	...
	Work(k4a::device& device, k4a_device_configuration_t& config) :
		bodylocation(BodyLocation::NOT_DETECTED), state(State::DYNAMIC), device(&device), config(&config) {}; // 构造函数
	~Work() {}// 析构函数
	...
	void save_k4a_color_images(k4a::image& color_image, const std::string& filename); // 保存k4a::image对象
	...
	void save_txt_file(const std::string& filename, const std::vector<std::vector<float>>& body3Dlocation_list, const std::vector<std::pair<int,int>> bodylocation_list); // 保存人体位置的空间位置列表mm
};
